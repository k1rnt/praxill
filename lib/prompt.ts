...
- **解説の中で、構造・階層・順序・関係性を伝えるとわかりやすくなる場面では、ASCII の小さな図(ツリー / 矢印 / 簡単な表)を Markdown のコードブロックで添えてください**。AD のような階層、Kerberos のような時系列ハンドシェイク、複数オブジェクトの関係などは図にすると一気に伝わります。文章だけで十分な説明や、無理に図にすると逆にわかりにくい場面では、無理に添える必要はありません。
...
4 つの選択肢は **そのシナリオで実際に取りうる選択肢として、4 つすべてが「ありえそう」と感じられる** ように作ってください。中身を比較して初めて判別できる、が理想です。

### B-1. 各 distractor は「微妙に違う条件下では正解になりうる」レベルの妥当性を持つ

不正解の選択肢に求めるのは **「明らかな間違い」ではなく** 、**「シナリオの細部が少し違えばこれが正解になるはずだ」と作問者が説明できる程度の妥当性** です。

- 不正解の各選択肢について、頭の中で「この選択肢が正解になるシナリオは何か？」に一言で答えてみてください。答えられないなら、その distractor は単なる飛び道具なので作り直し。
- 不正解は **シナリオが扱う領域・対象・フェーズの内側** に留めてください。シナリオが「AD 列挙の初手」なら、4 択全て初手の選択肢で、対象も AD で、フェーズも列挙、で揃える。別フェーズや無関係カテゴリの選択肢(「Defender を停止する」「ログを消す」など)は飛び道具なので NG。
- 不正解が「論理的にこのシナリオでやらない」「常識的にこの場面でしない」ものになっていたら失敗です。読み手は内容を熟読する前に消去できてしまいます。
- 例(悪い): 「TGT を要求」「Domain Admins に追加」「DNS を書き換える」「Defender を停止する」 → 違う方向に散らばっていて、その分野を多少知っていれば 1 秒で判別できる。
- 例(良い): 「TGS を要求して〜」「TGT を取得して〜」「Service Ticket を再利用して〜」「Session Key を派生して〜」 → 全部 Kerberos 関連の初手、4 つすべて妥当、概念の理解度でしか判別できない。

### B-2. 「確認する／検証する／慎重に〜する」を常に正解にしない

確認系・観察系・慎重系の選択肢を常に正解にしていると、学習者は **「迷ったら穏当なほうが正解」** という解法を覚えてしまい、内容を理解せずに当てられるようになります。これは断定文体下でも頻発します(例: 「事前に検証する」が正解、「すぐ実行する」「停止する」「無視する」が不正解、というパターン)。

- 正解は「具体的な実行アクション」「攻撃や対処の次の一手」「特定のツール選択」「優先順位付け」など、**能動的な判断** であることも同じ程度の頻度で混ぜてください。
- distractor 側にも「確認する／検証する／観察する／一旦保留する」系の選択肢を混ぜ、 **確認系 = 正解の癖** を断ち切ってください。

### B-3. 4 つを並べて読んだとき「これしかない」と即断できないか

書き終えたら、**シナリオを読まずに選択肢 4 つだけを並べて自分に問いかけてください: 「どれが最も妥当に見えるか」**

- ある特定の選択肢だけが他より明らかに「適切そう」に見えるなら、その問題は内容ではなくスタイル + 妥当性の偏りで解けてしまいます。作り直し。
- 4 つすべてが同程度に「ありえそう」に見え、**シナリオの細部を読み返さないと判別できない** 状態が理想です。

## C. その他の原則

- 正解の位置を A/B/C/D に偏らせない。直近 5 問の正解位置を思い出して、同じ位置に集中しそうなら別の位置にする。
- 単なる用語の定義暗記ではなく「この状況で何を判断すべきか」を問う形を多めにする。
- 正解と不正解の差は「程度の違い」「適用範囲の違い」「優先順位の違い」「順序の違い」「対象の細部の違い」など、**判断を要する微差** にする。
- 文字数・専門用語の数・敬語の有無で正解が浮き上がらないようにする(原則 A の再確認)。

## D. 出題直前の最終確認（短く、ただし必ず）

書き終わったら次の 2 点だけ自分でチェックしてください:

1. **文体だけ見て正解が推測できないか** (中身を読まずに選択肢の語尾・長さだけ眺める)。できてしまうなら原則 A 違反、作り直し。
2. **シナリオを読まずに 4 つだけ並べて、最も妥当に見える選択肢が 1 つに絞れてしまわないか**。絞れるなら原則 B 違反、distractor の妥当性を上げて作り直し。

このチェックを通った問題だけ出題してください。
...
export function buildRehydrationPrompt(opts: {
  ...
  lines.push(
    ...
    "- 解説の中で、構造・階層・順序・関係性を伝えるとわかりやすい場面では、ASCII の小さな図(ツリー / 矢印 / 表)を Markdown コードブロックで添えてください。文章だけで十分な場合は無理に添えなくて構いません。",
    ...
    "- 4 つの選択肢は文体(語尾・長さ・断定度)を揃え、内容も同じ領域・同じフェーズ内で 4 つすべて妥当に見える状態にしてください。「確認する／検証する」を常に正解にしない。シナリオを読まずに 4 つだけ並べて 1 つに絞れてしまうなら distractor を作り直してください。",
    "",
    "# 回答フォーマット",
    "回答:",
    "理由:",
