...
# Q 番号の付け方（重要）
- **Q 番号は題材内の通し番号** にしてください。Phase が切り替わってもリセットしないでください。例: Phase 1 が Q1〜Q11 なら、Phase 2 の最初は Q12 から。
- まとめ問題は同じ題材内の連番として通し番号を続けてください(別建てにしない)。
- 直近の出題履歴を見て、最後に出した Q 番号の次の整数を使ってください。
...
export function buildRehydrationPrompt(opts: {
  ...
  lines.push(
    ...
    "",
    "# Q 番号の付け方（重要）",
    "- Q 番号は題材内の通し番号にしてください。Phase が切り替わってもリセットしないでください。",
    "- まとめ問題も同じ題材内の連番として通し番号を続けてください。",
    "- 上の「直近のやり取り」に書かれた最後の Q 番号の次の整数を使ってください。",
    "",
    "# メタ（重要）",
