...
# 4 択を作る時の原則（生成時に守ること、最重要）

学習者が **内容ではなく文体・形式で正解を当てる** ことを完全に防いでください。以下は「後で見直す項目」ではなく、**最初から守って書く生成ルール** です。

## A. 4 つの選択肢の文体を揃える（違反禁止）

正解だけ hedge / 不正解は断定、のような **文体の非対称** が学習者にとって最大のヒントになります。これが起こりやすい典型ダメパターン:

- 正解だけが「〜の場合がある」「〜こともある」「〜可能性がある」「文脈による」のように推量で書かれ、不正解が「〜である」「必ず〜」「絶対に〜」のように断定で書かれている → 文体だけで「自信なさげな方が正解だな」と当てられてしまう。
- その逆: 正解だけ断定形で、不正解は妙に複雑な条件を付けている。

選択肢を書く時に以下を守ってください:

- 4 つの選択肢の **語尾の確信度・断定度・hedge 表現の有無を最初から揃える**。
  - 全部「〜である」で揃える、または全部「〜場合がある／〜こともある」で揃える、のどちらか一方に統一する。
- 正解にだけ「ただし」「場合がある」「文脈による」を入れ、不正解に入れない、は禁止。逆も禁止。
- 「絶対」「必ず」「一切」「すべて」のような **極端な語** を不正解にだけ入れない(正解候補にも入れるか、全部から削る)。
- 1 つだけ妙に丁寧 / 妙に専門用語多め / 妙に短い、のような **長さ・トーンの非対称** を作らない。
- 主語・述語の構造を 4 つで揃える。

## B. 内容で悩ませる（違反禁止）

4 つの選択肢は **同じ概念の角度違い・程度違い・適用範囲違い** で構成してください。

- 例(良い): 「TGS を要求して〜」「TGT を取得して〜」「Service Ticket を再利用して〜」「Session Key を派生して〜」 — 全部 Kerberos 関連、微妙に違う、学習者は概念の理解度で判別する必要がある。
- 例(悪い): 「TGT を要求」「Domain Admins に追加」「DNS を書き換える」「Defender を停止する」 — 違う方向に散らばっていて、その分野を多少知っていれば 1 秒で判別できる。

違う方向(全く別カテゴリ・別ツール・別フェーズ)に散らした選択肢を並べると、学習者は **違和感探しゲーム** になり、判断を問えません。

「**どっちもありそう、でも微妙にずれている**」状態を意図的に作ってください。distractor は、その分野を勉強中の学習者が「これも正しそう」と一瞬迷うレベルにしてください。

## C. その他の原則

- 正解の位置を A/B/C/D に偏らせない。直近 5 問の正解位置を思い出して、同じ位置に集中しそうなら別の位置にする。
- 単なる用語の定義暗記ではなく「この状況で何を判断すべきか」を問う形を多めにする。
- 正解と不正解の差は「程度の違い」「適用範囲の違い」「優先順位の違い」「順序の違い」など、判断を要するものにする。
- 不正解は「明らかに違う」「常識的にありえない」内容にしない。

## D. 出題直前の最終確認（短く）

書き終わったら 1 度だけ、選択肢の **中身を読まず文体だけ見て** 正解が推測できないか自分で試してください。できてしまうなら作り直してください。これだけで OK で、他のチェックリストは原則 A〜C を守っていれば満たされているはずです。
