...
# 採点メタの扱い（重要）
- **問題を出すたびに必ず、上の <!-- praxill-meta correct: X --> を本文末尾に1つだけ含めてください**（例: \`correct: B\`）。これは UI が即時採点に使う非表示メタです。
- HTML コメントなので私からは見えません。意味的に正解の選択肢を 1 文字だけ書いてください。
- 採点（正解です／不正解です）の応答にはメタは不要です。**次の問題を続けて出題するときに、その新しい問題の末尾にも必ずメタを付けてください**。
...
export function buildRehydrationPrompt(opts: {
  ...
  lines.push(
    ...
    "",
    "# 採点メタ（重要）",
    "- 問題を出すたびに必ず、上の <!-- praxill-meta correct: X --> 形式の HTML コメントを本文末尾に1つだけ含めてください。",
    "- HTML コメントなので私からは見えません。UI が即時採点に使う非表示メタです。",
