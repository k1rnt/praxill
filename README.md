...
> **セキュリティについて**
>
> Codex は `-s read-only -c approval_policy="never"` で起動している。承認ダイアログは出ない一方、サンドボックスは効いた状態で、もしモデルが書き込みやシェル実行を試みても読み取り以外は拒否される。Praxill 側でモデルにツール使用を求めない設計と合わせて、二重で安全側に倒している。
>
> 自分用 LAN 上のローカル利用を想定した個人ツールであり、認証も多人数対応も入っていない。 production や SaaS としてそのまま展開するなら、サンドボックスとは別軸で認証・テナント分離・コスト制御を追加する必要がある。
...
| `CODEX_REASONING` | `high` | 回答・解説ターンの reasoning effort (`medium` / `high`)。新規題材作成・要約・retry など「一度きりの作成系」呼び出しは別途 `xhigh` 固定で実行されます |
