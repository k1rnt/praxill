...
// The list covers the host's own name (auto-detected via os.hostname()),
// mDNS (.local), RFC1918 private ranges, and anything in
// PRAXILL_ALLOWED_DEV_ORIGINS (comma-separated) — useful when the box has
// multiple names or you proxy through a custom DNS.
const lanPatterns = [
  "*.local",
  "192.168.*.*",
  "10.*.*.*",
...
const extraOrigins = (process.env.PRAXILL_ALLOWED_DEV_ORIGINS ?? "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);

const nextConfig: NextConfig = {
  allowedDevOrigins: [hostname(), ...lanPatterns, ...extraOrigins],
